...
- **Fit on sample:** PCA is fitted on 50K rows (statistically identical to full data, since covariance stabilizes well before 50K) and applied to all 312K
...
**Inconsistent sub-cluster normalization:** The sklearn pipeline ran `StandardScaler().fit_transform()` independently inside each Spark partition during sub-commodity clustering. This means each partition learned its own mean and standard deviation, so points in different partitions of the same commodity cluster were normalized differently. The rustcluster pipeline normalizes all data once before clustering, ensuring consistency.
